...
- (void)imagePickerController:(UIImagePickerController *)picker
  ...
  if (videoURL != nil) {
    ...
    if (!originalAsset) {
      ...
      [self addGPS:info block:^(NSDictionary *resultInfo) {
          
        [weakSelf saveImageWithPickerInfo:resultInfo image:image imageQuality:imageQuality];
      }];
...
- (void )addGPS:(NSDictionary *)aInfo block:(void(^)(NSDictionary * resultInfo))excuteBlock{
    
    if (!aInfo) {
        return excuteBlock(aInfo);
    }
    
    NSMutableDictionary * pInfo = [NSMutableDictionary dictionaryWithDictionary:aInfo];
    
    [ImageLocationManager startLocationWithSuccessBlock:^(NSArray<CLLocation *> * _Nonnull locations) {
        ...
        if (pInfo && [pInfo objectForKey:UIImagePickerControllerMediaMetadata] && gpsDict) {
            
            NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:[pInfo valueForKey:UIImagePickerControllerMediaMetadata]];
            [dic  setValue:gpsDict forKey:(NSString*)kCGImagePropertyGPSDictionary];
            
            [pInfo setValue:dic forKey:UIImagePickerControllerMediaMetadata];
        }
...
//获取图片所有的信息
- (NSDictionary *)getImageAllInfo:(UIImage *)image{
